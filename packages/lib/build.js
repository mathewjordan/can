...
async function ensureSearchRuntime() {
  ...
  if (!esbuild) {
    console.warn("[build] esbuild not available; search will not be interactive.");
    return;
  }
  const entry = `
...
async function build() {
  ...
  // Ensure search page exists even if IIIF collection was unavailable
  try {
    const searchPath = path.join(OUT_DIR, 'search.html');
    if (!fs.existsSync(searchPath)) {
      // emit empty index and page
      const idxPath = path.join(OUT_DIR, 'search-index.json');
      await fsp.writeFile(idxPath, JSON.stringify([], null, 2), 'utf8');
...
async function buildIiifCollectionPages() {
  ...
  // Emit search index + page
  try {
    const idxPath = path.join(OUT_DIR, "search-index.json");
    await fsp.writeFile(idxPath, JSON.stringify(searchRecords, null, 2), "utf8");
    await ensureSearchRuntime();
    await buildSearchPage();
  } catch (e) {
    console.warn("Search: Failed to generate index/page:", e.message);
  }
}
