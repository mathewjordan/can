...
var IIIFHelpers = (() => {
  ...
  Object.freeze(o);
  Object.freeze(w);
  function P(e, { allowSourceString: n = true, allowString: i = false, allowedStringType: r } = {}) {
    let t = (a) => {
      if (n && a && a.source && typeof a.source != "string") {
        let s = Object.keys(a.source);
        if (a.source.id && a.source.type && s.length === 2)
          return { ...a, source: a.source.id };
      }
      return a;
    };
    if (e) {
      if (e.source && e.source.partOf)
        return t(e);
      let a = Object.keys(e);
      if (a.length === 2 && e.type && e.source || a.length === 3 && e.type && e.source && a.indexOf("selector") !== -1 && !e.selector)
        return i && (!r || r === e.source.type) ? e.source.id : e.source.type === "ContentResource" ? { type: "SpecificResource", source: e.source.id } : e.source;
      if (e.selector && !Array.isArray(e.selector) && typeof e.selector != "string" && e.selector.type === "FragmentSelector") {
        let s = `${e.source.id}#${e.selector.value}`;
        return i ? s : { id: s, type: e.source.type };
      }
    }
    return t(e);
  }
  ...
  return __toCommonJS(ranges_exports);
})();
